...
export class CodeBodyComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.requestService.downloadRequested.subscribe(() => {
      this.onDownload();
    });
  }
  ...
  toggleSearch() {
    ...
    document
      .getElementById('code-search-box')
      .classList.toggle('active-search');
  }

  onSearch(queryForm: NgForm) {
    if (
      queryForm.value.search.trim() === '' ||
      queryForm.value.search === null
    ) {
      document.getElementById(this.mode).textContent = this.requestInfo.body;
      return;
    }

    const searchQuery = queryForm.value.search.trim();
    const searchQueryLength = searchQuery.length;
    const searchStr = this.requestInfo.body;
    const indices = [];
    ...
    for (let i = 0; i < indices.length; i++) {
      searchArr.splice(
        indices[i] + (i - searchQueryLength * i),
        searchQueryLength,
        select
      );
    }
  ...
  onDownload() {
    ...
    const file = new Blob([this.requestInfo.body], {
      type: this.downloadFileType
    });
    const a = document.createElement('a'),
      url = URL.createObjectURL(file);
    a.href = url;
    a.download =
      this.requestInfo.url.split('.').join('') +
      Math.random()
        .toString(36)
        .substr(2, 6) +
      '.' +
  ...
  jQuery() {
    ...
    component.find('button:not(.drop-btn):not(.no-focus)').click(function() {
      component
        .find('button:not(.drop-btn):not(.no-focus)')
